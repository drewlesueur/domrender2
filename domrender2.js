...
$.renderWrap = function (type, fn) {
	return function (t, scope) {
		...
		fn(t, scope, newVal)
		var e = new CustomEvent('drchange' + type, {cancelable: false, bubbles: false, detail: {oldVal: t.oldVal, newVal: newVal, what: t.what}})
...
$.renderAccess = function (t, scope) { t.exprFn(scope, t.el) }
	...
	if (t.keyExprFn) {
		...
		for (var i=0; i<newVal.length; i++) {
			...
			if (extraData) { // clone a new extraData
				for (var k in extraData) { newExtraData[k] = extraData[k] }	
			}
			...
			if (oldStuff) {
				...
				newByKey[key] = { els: els, compiled: compiled }
				...
				var e = new CustomEvent('dradd', {cancelable: false, bubbles: false, detail: {index: i, value: val, list: newVal}})
		...
		for (var i=0; i<t.keys.length; i++) {
			...
			if (!newCached) {
				...
				for (j=0; j<oldCached.els.length; j++) {
					...
					if (el.parentNode) {
						var e = new CustomEvent('drremove', {cancelable: false, bubbles: false, detail: {index: i, list: newVal}})
						events.push([el, e])
		...
		for (var i=t.curLen; i<newVal.length; i++) { // add new ones
			...
			// TODO: potentially trigger on all children, like you do with remove
			var e = new CustomEvent('dradd', {cancelable: false, bubbles: false, detail: {index: i, value: newVal[i], list: newVal}})
			events.push([firstChild, e])
		}
...
$.compile = function (el, parentD) {
	var d = { boundThings: [], root: (parentD && parentD.root), lastIf: null, lastSwitch: null}
	if (!parentD) { d.root = d }
	$.visit(el, d)
	return d
}
...
$.addBookkeepingBoundThing = function (child, d) {
	d.boundThings.push({ type: "general", render: $.renderGeneral, el: child })
}
$.parseInterpolated = function (str) {
	var parts = [], look = "{", start = 0
	var lastStart = 0
...
$.addInterpolatedPart = function (parts, str, start, end, look) {
	...
	if (part.length > 0) {
		var iPart = look == "{" ? {type: look, expr: part} : {type: look, expr: part, exprFn: $.compileExpr(part)}
		parts.push(iPart)
	}
}
		...
		if (child.nodeValue.indexOf("{") != -1) { // if its interpolation syntax // is there a faster first check 
			...
			if (parts.length) {
                ...
                if (parts.length == 1) {
                    ...
                    d.boundThings.push({ type: "{}", render: $.renderInterpolatedFast, exprFn: parts[0].exprFn, el: child })	
                } else {
                    d.boundThings.push({ type: "{}", render: $.renderInterpolated, parts: parts, el: child })	
                }
			}
		}
...
$.compileSubActions = {
					"@style": function (parts0, parts1, d, child, attr, attrName) {
						if (attrName == "@style") { parts1 = $.camelCase(parts1) }
					...
					"@class": function (parts0, parts1, d, child, attr, attrName) {
						d.boundThings.push({ type: "class", render: $.renderBasic[parts0], exprFn: $.compileExpr(attr.value), what: parts1, el: child })
                    },
					"@attr": function (parts0, parts1, d, child, attr, attrName) {
						d.boundThings.push({ type: "attr", render: $.renderBasic[parts0], exprFn: $.compileExpr(attr.value), what: parts1, el: child })
                    },
					"@hasattr": function (parts0, parts1, d, child, attr, attrName) {
						d.boundThings.push({ type: "hasattr", render: $.renderBasic[parts0], exprFn: $.compileExpr(attr.value), what: parts1, el: child })
                    },
...
$.compileActions = {
				...
				"@exec": function (d, child, attr, attrName) {
					d.boundThings.push({ type: "@exec", render: $.renderExec, el: child, expr: attr.value, exprFn: $.compileExpr(attr.value) })
                },
				"@debugger": function (d, child, attr, attrName) {
					d.boundThings.push({ type: "@debugger", render: $.renderDebugger, el: child, expr: attr.value, exprFn: $.compileExpr(attr.value) })
                },
				...
				"@repeat": function (d, child, attr, attrName) {
					...
					var keyExprFn
					if (keyExpr) { keyExprFn = $.compileExpr(keyExpr)}
				...
				"@access": function(d, child, attr, attrName) {// make the scope aware of this element 
					...
					d.boundThings.push({ render: $.renderAccess, exprFn: fn, el: child })	
                },
				...
				"@if": function (d, child, attr, attrName) { return $.compileCond(attrName, attr.value, child, d) },
				"@elseif": function (d, child, attr, attrName) { return $.compileCond(attrName, attr.value, child, d) },
				"@else": function (d, child, attr, attrName) { return $.compileCond(attrName, attr.value, child, d) },
				"@case": function (d, child, attr, attrName) { return $.compileCond(attrName, attr.value, child, d) },
				"@default": function (d, child, attr, attrName) { return $.compileCond(attrName, attr.value, child, d) },
}
...
$.findEndEls = function (child) {
	...
	if (child.getAttribute("@start") !== null) {
		...
		while (ch = ch.nextSibling) {
			...
			if (ch.getAttribute("@start") !== null)	{ startCount += 1 }
			if (ch.getAttribute('@end') !== null)	{ startCount -= 1 }
	...
	if (child.getAttribute("@wrapstart") !== null) {
		...
		var startCount = 1
		while (ch = ch.nextSibling) {
			if (ch.nodeType != 1) 			{ forFrag.push(ch); continue; }
			
			if (ch.getAttribute("@wrapstart") !== null)	{ forFrag.push(ch); startCount += 1 }
			if (ch.getAttribute('@wrapend') !== null)	{ startCount -= 1 }
			if (startCount == 0)			{ ch.parentNode.removeChild(ch); break } else {forFrag.push(ch)}
		}
		//child.parentNode.removeChild(child)	
	}
	return forFrag
...
$.attachInput = function (d, child, expr, boundThing) {
	...
	child.addEventListener(inputOrChange, function (e) {
		...
		var e = new CustomEvent('drchange' + 'b', {cancelable: false, bubbles: false, detail: {oldVal: boundThing.oldVal, newVal: value}})
...
$.addHtml = function (parentEl, urls, finalCb) { // load external html like you would scripts
	...
	var cb = function (err, response, xhr) {
		...
		count += 1
		if (count == urls.length) { finalCb(null) }
	}
	for (var i=0; i<urls.length; i++) { $.ajax('GET', urls[i], "", cb) }	
}
$.ajax = function (method, url, data, cb) {
	...
	request.onerror = function(err) { cb (err, this.response, this) };
	request.send(data);
}
return $
})({})
